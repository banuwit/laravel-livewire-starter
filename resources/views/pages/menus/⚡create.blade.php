...
new class extends Component
{
    ...
    {
        $this->parentMenus = Menu::where('level', 0)->orderBy('sort_order')->get(['id', 'name'])->toArray();
    }
    ...
    <form id="save-form" wire:submit="createMenu" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <flux:tab.group variant="segmented">
                    ...
                    <flux:tab.panel name="permissions" class="space-y-3">
                        <flux:text size="sm" class="!text-zinc-500">
                            ...
                            Each permission belongs strictly to this menu. Use the convention <code class="text-xs px-1 rounded bg-zinc-100 dark:bg-white/10">{{ $slug ?: 'slug' }}.action</code> (e.g. <code class="text-xs px-1 rounded bg-zinc-100 dark:bg-white/10">{{ $slug ?: 'users' }}.view</code>).
                        </flux:text>
                        ...
                        @if (!empty($permissions))
                            <div class="rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900/40 divide-y divide-zinc-200 dark:divide-zinc-700">
                                @foreach ($permissions as $i => $perm)
                                    <div class="flex items-start gap-2 p-3" wire:key="perm-{{ $i }}">
                                        <div class="flex-1">
                                            <flux:input
        ...
        <div class="flex flex-col gap-6 lg:order-1">
            <flux:card class="space-y-5">
                ...
                <flux:input wire:model="slug" label="Slug" placeholder="auto from name" description="Used as permission prefix" />
                <flux:input wire:model="icon" label="Icon" placeholder="e.g. user, cog-6-tooth" description="Heroicon name" />
