...
<?php if ($expandable && $heading): ?>
    <?php if ($icon): ?>
        <ui-disclosure {{ $attributes->class('group/disclosure in-data-flux-sidebar-collapsed-desktop:hidden') }} @if ($expanded === true) open @endif data-flux-sidebar-group>
            <button type="button" class="border-1 border-transparent w-full h-8 in-data-flux-sidebar-on-mobile:h-10 flex items-center group/disclosure-button my-px rounded-lg text-white/70 hover:text-white hover:bg-white/10 dark:hover:bg-white/[7%] cursor-pointer">
                <div class="px-3">
                ...
                <span class="flex-1 text-left rtl:text-right text-sm font-medium leading-none text-white/70">{{ $heading }}</span>
            ...
            <div class="relative hidden data-open:block ps-7" @if ($expanded === true) data-open @endif>
                <div class="absolute inset-y-[3px] w-px bg-white/30 dark:bg-white/30 start-0 ms-5"></div>
        ...
        <flux:dropdown hover class="in-data-flux-sidebar-on-mobile:hidden not-in-data-flux-sidebar-collapsed-desktop:hidden" position="right" align="start" data-flux-sidebar-group-dropdown>
            <button type="button" class="border-1 border-transparent w-full px-3 in-data-flux-menu:px-2 h-8 flex gap-3 items-center group/disclosure-button my-px rounded-lg in-data-flux-sidebar-collapsed-desktop:not-in-data-flux-menu:w-10 in-data-flux-sidebar-collapsed-desktop:not-in-data-flux-menu:justify-center hover:bg-white/10 dark:hover:bg-white/[7%] in-data-flux-menu:hover:bg-zinc-50 dark:in-data-flux-menu:hover:bg-zinc-600 text-white/70 in-data-flux-menu:text-zinc-800 hover:text-white dark:text-white/80 in-data-flux-menu:dark:text-white dark:hover:text-white">
                <?php if ($icon): ?>
                    <div class="relative">
                        <?php if (is_string($icon) && $icon !== ''): ?>
                            <flux:icon :icon="$icon" :variant="$iconVariant" class="text-white/70 dark:text-white/80 in-data-flux-menu:text-white/70 in-data-flux-menu:dark:text-white/80 [[data-flux-sidebar-group-dropdown]>button:hover_&]:text-current in-data-flux-menu:[[data-flux-sidebar-group-dropdown]>button:hover_&]:text-current size-4" />
                        <?php else: ?>
                            {{ $icon }}
                        <?php endif; ?>
                    </div>
                ...
                <span class="hidden in-data-flux-menu:block flex-1 text-start text-sm font-medium leading-none text-white text-zinc-800 dark:text-white">{{ $heading }}</span>
    ...
    <?php else: ?>
        <ui-disclosure {{ $attributes->class('group/disclosure in-data-flux-sidebar-collapsed-desktop:hidden') }} @if ($expanded === true) open @endif data-flux-sidebar-group>
            <button type="button" class="border-1 border-transparent w-full h-8 in-data-flux-sidebar-on-mobile:h-10 flex items-center group/disclosure-button my-px rounded-lg hover:bg-zinc-800/5 dark:hover:bg-white/[7%] text-zinc-500 hover:text-zinc-800 dark:text-white/80 dark:hover:text-white">
                <span class="text-sm font-medium leading-none text-white/70">{{ $heading }}</span>

                <div class="ps-3.5 pe-3.5 ms-auto">
                    <flux:icon.chevron-down class="size-3! hidden group-data-open/disclosure-button:block" />
                    <flux:icon.chevron-right class="size-3! block group-data-open/disclosure-button:hidden rtl:rotate-180" />
                </div>
            </button>
