...
$iconClasses = Flux::classes('text-white/70 dark:text-white/80 group-hover:text-white dark:group-hover:text-white')
...
$classes = Flux::classes()
    ...
    ->add('p-1 hover:bg-white/[7%] dark:hover:bg-white/[7%] cursor-pointer')
    ;
@endphp
...
<button type="button" {{ $attributes->class($classes) }} data-flux-sidebar-profile>
    ...
    <?php if ($name): ?>
        <span class="in-data-flux-sidebar-collapsed-desktop:hidden mx-2 text-sm text-white/70 hover:text-white dark:text-white group-hover:text-white dark:group-hover:text-white font-medium truncate">
            {{ $name }}
        </span>
    <?php endif; ?>
